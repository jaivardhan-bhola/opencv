...
public:
    ...
    /// @param blob Decoded (not base64) SentencePiece precompiled-charsmap bytes; may be empty.
...
private:
    ...
    // falling back to unkId_ (never repeated back-to-back) for any span with
    // no vocab match.
    ...
    // leading UNIGRAM_METASPACE marker (the reference SentencePiece "▁" word
    // boundary), and appends the resulting ids.
