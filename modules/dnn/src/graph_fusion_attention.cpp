...
struct ModelFusionAttention
{
    ...
    // Walks the "shape-computation" chain feeding a reshape/matmul target
    // shape argument (Concat/Gather2/Unsqueeze/Shape nodes) and marks every
    // node on it for removal, PROVIDED every consumer of that node's output
    // is either `owner_idx` (the op that is about to replace/absorb this
    // chain) or already part of `chain`. Without this check a shape-producing
    // node with an extra, still-live consumer elsewhere in the graph would be
    // deleted anyway, leaving that consumer's input pointing at a
    // producer-less argument.
